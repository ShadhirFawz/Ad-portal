...
function ProfileContent() {
  ...
  useEffect(() => {
    ...
    if (!USERNAME_REGEX.test(clean)) {
      setUsernameAvailability({
        ...
        available: false,
        valid: false,
        message: "Letters, numbers, underscores, and hyphens only (cannot start/end with symbols).",
      });
      setCheckingUsername(false);
      return;
    }
  ...
  const handleSaveUsername = async (e: FormEvent) => {
    ...
    if (usernameAvailability && (!usernameAvailability.valid || !usernameAvailability.available)) {
      setUsernameError(usernameAvailability.message);
      return;
    }
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center">
              <div className="relative flex-1">
                ...
                <div className="absolute right-3.5 top-1/2 -translate-y-1/2 flex items-center pointer-events-none">
                  ...
                  ) : usernameInput.trim() !== "" && usernameAvailability?.available && usernameAvailability?.valid ? (
                    <CheckCircle2 className="w-4 h-4 text-emerald-500 animate-in zoom-in duration-150" />
                  ) : usernameInput.trim() !== "" && usernameAvailability && (!usernameAvailability.available || !usernameAvailability.valid) ? (
                    <XCircle className="w-4 h-4 text-rose-500 animate-in zoom-in duration-150" />
                  ) : null}
                </div>
              </div>
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white text-xs font-bold transition shadow-md shadow-emerald-600/20 whitespace-nowrap cursor-pointer disabled:cursor-not-allowed"
              >
                {usernameSaving ? (
                  <>
                  ...
                  <>
                    <span>{redirectParam ? "Confirm" : "Confirm"}</span>
            ...
            <div className="min-h-5 flex items-center">
              {checkingUsername ? (
                ...
                <p className="text-[11px] text-slate-500 dark:text-slate-400">
                  Must be 3–30 characters using letters, numbers, hyphens, or underscores.
                </p>
              )}
            </div>
          </form>

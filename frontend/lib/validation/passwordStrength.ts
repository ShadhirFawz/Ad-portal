...
export function getPasswordStrength(pw: string): PasswordStrength {
  if (!pw) return { score: 0, label: "", color: "bg-slate-200 dark:bg-slate-700", width: "0%" };
  let score = 0;
  ...
  if (score <= 1) return { score, label: "Very Weak", color: "bg-rose-500", width: "20%" };
  ...
  if (score === 4) return { score, label: "Strong", color: "bg-emerald-400", width: "80%" };
